...
@Composable
fun WellsScreen(
    viewModel: WellsScreenViewModel = hiltViewModel(),
    reportFilters: ReportFilters,
    onBackPressed: () -> Unit,
    onSaveWells: (List<Well>) -> Unit,
) {
    val wells by viewModel.wells.collectAsState(initial = listOf())
    val selectedWells by viewModel.selectedWells.collectAsState(initial = emptyList())
    val isLoading by viewModel.isLoading.collectAsState(initial = false)
    val searchText by viewModel.searchText.collectAsState()

    if (reportFilters.wells.isNotEmpty()) {
        viewModel.initSelectedFields(reportFilters.wells)
    }
            ...
            TopAppBar(
                title = {
                    Row(
                        ...
                        TextButton(
                            onClick = {
                                val newWells = viewModel.resetSelectedWells()
                                reportFilters.wells = newWells
                            },
        ...
        Box(
            ...
            Column(
                ...
                if (isLoading) {
                    ...
                    val filteredFields = wells?.filter { well ->
                        well.wellNumber.contains(searchText, ignoreCase = true)
                    }
                    ...
                    LazyColumn(
                        ...
                        filteredFields?.let { fieldsList ->
                            itemsIndexed(fieldsList) { index, well ->
                                WellItem(
                                    well = well,
                                    isSelected = selectedWells.contains(well),
                                    onSelect = { viewModel.toggleFieldSelection(well) },
