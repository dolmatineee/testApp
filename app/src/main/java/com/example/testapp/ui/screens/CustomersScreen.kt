...
import com.example.testapp.ui.viewmodels.CustomersScreenViewModel
...
@Composable
fun CustomersScreen(
    viewModel: CustomersScreenViewModel = hiltViewModel(),
    reportFilters: ReportFilters,
    onBackPressed: () -> Unit,
    onSaveCustomers: (List<Customer>) -> Unit,
) {
    val customers by viewModel.customers.collectAsState(initial = listOf())
    val selectedCustomers by viewModel.selectedCustomers.collectAsState(initial = emptyList())
    val isLoading by viewModel.isLoading.collectAsState(initial = false)
    val searchText by viewModel.searchText.collectAsState()

    if (reportFilters.customers.isNotEmpty()) {
        viewModel.initSelectedCustomers(reportFilters.customers)
    }
            ...
            TopAppBar(
                title = {
                    Row(
                        ...
                        TextButton(
                            onClick = {
                                val newCustomers = viewModel.resetSelectedCustomers()
                                reportFilters.customers = newCustomers
                            },
            ...
            Column(
                ...
                if (isLoading) {
                    ...
                    val filteredCustomers = customers?.filter { customer ->
                        customer.companyName.contains(searchText, ignoreCase = true)
                    }
                    ...
                    LazyColumn(
                        ...
                        filteredCustomers?.let { customersList ->
                            itemsIndexed(customersList) { index, customer ->
                                CustomerItem(
                                    customer = customer,
                                    isSelected = selectedCustomers.contains(customer),
                                    onSelect = { viewModel.toggleCustomerSelection(customer) },
                                    isLastItem = index == customersList.lastIndex
                                )
                            }
                        }
                    }
